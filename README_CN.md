...
sk watch           实时监听运行时目录中新 skill 的出现
```
...
实时监听运行时 skills 目录中新 skill 的出现：

```bash
sk watch                  # 监听所有已配置的运行时
sk watch --runtime claude # 仅监听指定运行时
```

基于轮询检测（每 2 秒）——跨平台可靠。当出现包含 `SKILL.md` 的新 skill 目录时，输出：

```
✨ 发现新 skill: my-skill — 使用 sk install ~/.claude/skills/my-skill 安装
```

持续运行直到 `Ctrl+C`。收到 `SIGINT` 时优雅退出。
...
当传入 `--json` 时，输出一个 JSON 对象，包含 `runtimes`、`central`、`duplicates`、`totalDuplicatesCount`、`conflictsCount`、`savingsBytes` 和 `incompleteTransactions` 字段——适合通过管道传给 `jq` 或被其他工具消费。
...
```bash
sk watch                  # 监听所有已配置的运行时
sk watch --runtime <name> # 仅监听指定运行时
```

| 选项 | 别名 | 说明 |
|------|------|------|
| `--runtime <name>` | `-r` | 仅监听指定运行时。省略则监听所有已配置的运行时。 |

每 2 秒轮询一次。当出现包含 `SKILL.md` 的新 skill 目录时打印通知。持续运行直到 `Ctrl+C`。
...
```
src/
├── index.ts              # Commander CLI 入口（6 个子命令）
├── commands/
│   ├── doctor.ts         # 扫描、分析、报告（人类可读 + JSON）
│   ├── dedupe.ts         # 迁移（带回滚）+ 交互式 diff
│   ├── link.ts           # 软链接管理
│   ├── list.ts           # 树状可视化
│   ├── install.ts        # Skill 安装
│   ├── uninstall.ts      # Skill 移除
│   └── watch.ts          # 实时目录监听
├── core/
